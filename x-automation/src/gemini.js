 ...
 * Generates unique, data-driven wealth building and crypto tweets using Google Gemini API
 * Features: Real-time data, source linking, diverse sub-niches
 */
...
/**
 * Generate a unique, data-driven wealth building/crypto tweet using Gemini
...
export async function generateTechTweet(theme = "") {
  ...
  try {
    ...
    const prompt = `You are a data-driven contrarian expert who shares real, up-to-date insights about Crypto + Investing + Business + AI + Money psychology + Discipline.
...
CORE NICHE: Crypto + Investing + Business + AI + Money psychology + Discipline.
, anti-hype wealth building focused on Crypto (BTC/ETH) + disciplined investing + entrepreneurship
...
Return ONLY the tweet text with source attribution. Make it scroll-stopping and data-rich.`;
    ...
    console.log(
      `[INFO] Generating data-driven tweet #${randomSeed} for theme: "${theme || "General"}"...`,
    );
    ...
    tweetText = tweetText.replace(/^Tweet:\s*/i, "").trim();

    // Ensure it's within character limits (50-270)
    if (tweetText.length < 50) {
      console.warn(`[WARNING] Tweet too short (${tweetText.length} chars). Regenerating...`);
    ...
    console.log(`[SUCCESS] Data-driven tweet generated! Length: ${tweetText.length} chars`);
    return tweetText;
  } catch (error) {
    console.error(
...
/**
 * Generate multiple unique, data-driven tweets with varied angles and sub-niches
...
Format: Return ONLY the ${count} tweets, numbered 1-${count}, one per line.
No quotes around tweets, no explanations, no extra commentary.
Each tweet must be complete, data-rich, and within character limits.`;

    console.log(`[INFO] Generating batch of ${count} data-driven tweets (Batch #${batchId})...`);
    ...
    let tweets = text
      ...
      .filter((line) => line.length >= 50 && line.length <= 270);
    ...
    // Filter to ensure all tweets meet length requirements
    tweets = tweets.filter(tweet => tweet.length >= 50 && tweet.length <= 270);

    console.log(`[SUCCESS] Generated ${tweets.length} data-driven tweets in batch ${batchId}!`);
